...
Subscribes to parsed next entry messages and writes them to disk
"""
...
async def main() -> None:
    ...
    try:
        counter = 0
        print(f"✍️  Next writer started, listening on '{INPUT_TOPIC}'...")
        ...
        async def handler(msg):
            ...
            try:
                ...
                print(f"✓ Wrote {output_file.name} ({projects_count} project(s), date: {date_str}, note_id: {source_note_id})")
                counter += 1
